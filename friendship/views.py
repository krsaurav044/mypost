...
def view_friends(request, username, template_name='friendship/friend/user_list.html'):
    ...
    return render(request, template_name, {
        ...
        'friendship_context_object_name': get_friendship_context_object_name(),'friends':friends
    })
...
def friendship_add_friend(request, to_username, template_name='friendship/friend/add.html'):
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            return redirect('friendship:friendship_request_list',username=from_user.username)

    return render(request, template_name, ctx)


def friendship_remove_friend(request,to_username,template_name='remove.html'):
    ctx={'to_username':to_username}
    if request.method=='POST':
        to_user=user_model.objects.get(username=to_username)
        from_user=request.user
...
def friendship_reject(request, friendship_request_id):
    ...
    if request.method == 'POST':
        f_request = get_object_or_404(
            ...
            id=friendship_request_id)
        f_request.reject()
        return redirect('friendship:friendship_request_list',username=request.user.username)
...
@login_required
def friendship_request_list(request,username, template_name='friendship/friend/requests_list.html'):
    """ View unread and read friendship requests """
    # friendship_requests = Friend.objects.requests(request.user)
    user=get_object_or_404(user_model,username=username)
    friendship_requests = FriendshipRequest.objects.filter(rejected__isnull=True,to_user=user)
...
def all_users(request,template_name="friendship/user_actions.html"):
    ...
    users = user_model.objects.exclude(friends__from_user=request.user).exclude(id=request.user.id)
...
def block_add(request, blocked_username, template_name='friendship/block/add.html'):
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            return redirect('friendship:friendship_blocking', username=blocker.username)
